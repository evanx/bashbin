...
where two command-line arguments are specified viz. `scan` and `article:*` - and so we invoke the function `c1scan` - since the "command" is "scan," and there is one argument for that viz. `article:*`
